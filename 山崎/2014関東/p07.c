...
void led2(void);	//2つのLEDを同時に暗く表示させる関数
...
void main(void){
	...
	while(1){
		...
		if(bs == 0 && f == 1 && f2 == 1 && f3 == 1){	//カウントアップ動作
			f = 0;
			b++;
		...
		if(ps == 0 && f3 == 1){	//フォトICが遮光されたときの動作
			...
			for(c=0;c<b;c++){	//ステッピングモータの回転回数を判断
				for(c2=0;c2<10;c2++){
					for(c3=0;c3<48;c3++){
						stmr();
						led();
...
//=====変数の初期化
	a = 18;	//左の7セグLメントLEDのデータを保持
	b = 10;	//右の7セグLメントLEDのデータを保持
	c = 0;	//カウント用
	c2 = 0;	//カウント用
	c3 = 0;	//カウント用
	i = 0;	//ステッピングモータ制御用
