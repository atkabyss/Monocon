...
//=====変数の初期化
	...
	while(1){
		LATD = l7s[b];			//-----右側の7セグメントLEDのデータを読み込み
		l3 = 0;					//-----右側の7セグメントLEDを点灯
		
		if(bs == 1 && ys == 1){		//-----条件：両方のスイッチがOFFのとき
			b = 0;						//-----右側の7セグメントLEDに"0"を表示
		}
		if(bs == 1 && ys == 0){		//-----条件：黄色タクトスイッチのみがONのとき
			b = 1;						//-----右側の7セグメントLEDに"1"を表示
		}
		if(bs == 0 && ys == 1){		//-----条件：青タクトスイッチのみがONのとき
			b = 2;						//-----右側の7セグメントLEDに"2"を表示
		}
		if(bs == 0 && ys == 0){		//-----条件：両方のスイッチがONのとき
			b = 3;						//-----右側の7セグメントLEDに"3"を表示
		}
