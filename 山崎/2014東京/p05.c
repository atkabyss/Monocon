...
//=====変数の初期化
	a = 8;		//-----左側の7セグメントLEDのデータを保持
	b = 5;		//-----右側の7セグメントLEDのデータを保持
	c = 0;		//-----カウント用
	i = 0;		//-----ステッピングモータ制御用
	...
	while(1){
		if(k == 0){			//-----プログラム開始時の処理
			k = 1;
			LATD = l7s[18];		//-----パターン1を読み込み
			l2 = 0;				//-----左側の7セグメントLEDを点灯
		}
		if(ys == 0){		//-----条件：黄色タクトスイッチがONのとき
			f = 1;				//-----イルミネーションの実行
		}
		if(ys == 1){		//-----条件：黄色タクトスイッチがONのとき
			f = 0;				//-----イルミネーションを停止
		}
		
		if(f == 1 && f2 == 0){			//-----条件：イルミネーションが実行されているとき
			//=====1の状態
			l3 = 1;				//-----右側の7セグメントLEDを消灯
			LATD = l7s[18];		//-----イルミネーションのパターン1を読み込み
			l2 = 0;				//-----左側の7セグメントLEDを点灯
			for(c=0;c<10;c++){	//-----約1秒の遅延処理
				D10;
				ils();			//-----スイッチの状態をチェック
			}
			f2 = 1;				//-----2の状態を実行
		}
		if(f == 1 && f2 == 1){			//-----条件：イルミネーションが実行されているとき
			//=====2の状態
			l2 = 1;				//-----右側の7セグメントLEDを消灯
			LATD = l7s[18];		//-----イルミネーションのパターン1を読み込み
			l3 = 0;				//-----右側の7セグメントLEDを点灯
			for(c=0;c<10;c++){	//-----約1秒の遅延処理
				D10;
				ils();			//-----スイッチの状態をチェック
			}
		...
		if(f == 1 && f2 == 2){			//-----条件：イルミネーションが実行されているとき
			//=====3の状態
			l2 = 1;				//-----右側の7セグメントLEDを消灯
			LATD = l7s[19];		//-----イルミネーションのパターン2を読み込み
			l3 = 0;				//-----右側の7セグメントLEDを点灯
			for(c=0;c<10;c++){	//-----約1秒の遅延処理
				D10;
				ils();			//-----スイッチの状態をチェック
		...
		if(f == 1 && f2 == 3){			//-----条件：イルミネーションが実行されているとき
			//=====4の状態
			l3 = 1;				//-----右側の7セグメントLEDを消灯
			LATD = l7s[19];		//-----イルミネーションのパターン2を読み込み
			l2 = 0;				//-----右側の7セグメントLEDを点灯
			for(c=0;c<10;c++){	//-----約1秒の遅延処理
				D10;
				ils();			//-----スイッチの状態をチェック
			}
