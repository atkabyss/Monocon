...
while(1){
	...
	if(HSW == 0 && TGSW == 0){ //トグルスイッチがHかつフォトインタラプタが遮断のとき高速回転
		dcmr();
	}
	
	if(HSW == 0 && TGSW == 1){ //トグルスイッチがLかつフォトインタラプタが遮断のとき低速回転
		dcmrs();
		D10k;
		D10k;
		D10k;
	...
	if(HSW == 1){ //フォトインタラプタが透過のときDCモータを停止
		dcms();
	}
}
}
